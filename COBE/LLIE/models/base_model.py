...
class BaseModel():
    def __init__(self, opt):
        self.opt = opt
        self.device = paddle.set_device('gpu' if opt['gpu_ids'] is not None else 'cpu')
    ...
    def _get_init_lr(self):
        ...
        for optimizer in self.optimizers:
            init_lr_groups_l.append([v['learning_rate'] for v in optimizer._param_groups])
    ...
    def get_current_learning_rate(self):
        return [param_group['lr'] for param_group in self.optimizers[0]._param_groups]
    ...
    def load_network(self, load_path, network, strict=True):
        ...
        for k, v in load_net.items():
            load_net_clean[k] = v  # No prefix in Paddle
        network.set_state_dict(load_net_clean)
