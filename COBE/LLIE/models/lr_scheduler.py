...
class MultiStepLR_Restart(LRScheduler):
    ...
    def get_lr(self):
        if self.last_epoch in self.restarts:
            if self.clear_state:
                self.optimizer._state = defaultdict(dict)
            weight = self.restart_weights[self.restarts.index(self.last_epoch)]
            return [group['initial_lr'] * weight for group in self.optimizer._param_groups]
        if self.last_epoch not in self.milestones:
            return [group['lr'] for group in self.optimizer._param_groups]
        return [
    ...
    def get_lr(self):
        if self.last_epoch == 0:
            ...
            return [
                ...
                for base_lr, group in zip(self.base_lr, self.optimizer._param_groups)
            ]
        return [
