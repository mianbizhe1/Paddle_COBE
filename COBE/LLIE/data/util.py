...
def _read_img_lmdb(env, key, size):
    """Read image from LMDB with key (with and without fixed size)
    size: (C, H, W) tuple"""
...
def read_img_seq(path, size=None):
    ...
    imgs = paddle.to_tensor(np.ascontiguousarray(np.transpose(imgs, (0, 3, 1, 2)))).astype(paddle.float32)
    return imgs
